...
pub fn boids_ui(config: &mut BoidsConfig, ui: &mut Ui) {
    ...
    ui.end_row();
    ui.add(egui::Slider::new(&mut config.align_factor, 0.000..=1.0).text("Align factor"));
    ui.end_row();
    ui.add(egui::Slider::new(&mut config.avoid_factor, 0.000..=0.1).text("Avoid factor"));
    ui.end_row();
    ui.add(egui::Slider::new(&mut config.centering_factor, 0.005..=5.0).text("Centering factor"));
    ui.end_row();
